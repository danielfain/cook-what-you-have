<script lang="ts">
    ...
    import { contains, select } from "$lib/stores/selected-ingredients.svelte";

    let selectedCategory = $state<Category>();
    let isModalOpen = $state(false);

    let selectedIngredient = $state<Ingredient>();
    let isSelectingMeasurement = $state(false);
    ...
    const categoryIngredients: Ingredient[] = $derived.by(() => {
        ...
        return availableIngredients.filter(
            (i) => i.categoryId === selectedCategory?.id,
        );
    });

    function save(ingredient: Ingredient, amount: number, unit: Unit) {
        select(ingredient, amount, unit);
        isSelectingMeasurement = false;
    }

    function getCategoryIngredientCount(categoryId: string): number {
        return availableIngredients.filter((i) => i.categoryId === categoryId)
            .length;
    }

    function handleIngredientClick(ingredient: Ingredient): void {
        selectedIngredient = ingredient;
        isSelectingMeasurement = true;
    }
...
<h1 class="text-xl font-bold">Browse Ingredients</h1>
...
>
    <DialogContent class="max-w-3xl">
        ...
        <ScrollArea class="h-[500px] pr-4">
            <div
                class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 pb-4"
            >
                {#each categoryIngredients as ingredient}
                    {@const isSelected: boolean = contains(ingredient.id)}
                    ...
                    >
                        ...
                        >
                            <div
                                class="p-4 flex flex-col items-center text-center gap-3"
                            >
                                {#if isSelected}
                                    <div class="absolute top-2 right-2">
                                        <div
                                            class="bg-primary text-primary-foreground rounded-full p-1"
                                        >
                                            <Check class="w-4 h-4" />
                                        </div>
                                    </div>
        ...
        {#if selectedIngredient}
            <Dialog
                open={isSelectingMeasurement}
                closeOnEscape
                closeOnOutsideClick
                onOpenChange={() => (isSelectingMeasurement = false)}
            >
                <DialogContent>
                    <MeasurementSelector
