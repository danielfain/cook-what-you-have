...
export function select(
  ...
  const alreadySelected = selectedIngredients.some(
    (selectedIngredient) => selectedIngredient.ingredient.id === ingredient.id,
  );
  ...
  if (!alreadySelected) {
    ...
    console.error(
      "Tried to select an ingredient previously selected. Should not be possible.",
    );
  }
}
...
export function remove(ingredientId: string): void {
  selectedIngredients = selectedIngredients.filter(
    (selectedIngredient) => selectedIngredient.ingredient.id !== ingredientId,
  );
}
